...
type LiveBufferInspectorConfig struct {
	Warn  time.Duration
	Error time.Duration
}

func DefaultLiveBufferInspectorConfig() *LiveBufferInspectorConfig {
	return &LiveBufferInspectorConfig{
		Warn:  2 * time.Second,
		Error: 0,
	}
}

func NewLiveBufferInspector() core.DASHInspector {
	return NewLiveBufferInspectorWithConfig(DefaultLiveBufferInspectorConfig())
}

func NewLiveBufferInspectorWithConfig(config *LiveBufferInspectorConfig) core.DASHInspector {
	return &liveBufferInspector{
		config,
	}
}

type liveBufferInspector struct {
	config *LiveBufferInspectorConfig
}

func (ins *liveBufferInspector) Inspect(manifest *core.Manifest, segments core.SegmentStore) *core.Report {
	if manifest.Type != nil && *manifest.Type == "static" {
		return &core.Report{
			Name:     "LiveBufferInspector",
			Severity: core.Info,
	...
	err := manifest.EachSegments(func(segment *core.DASHSegment) (cont bool) {
		...
		f := periodStart + float64(t)/timescale
		tm := time.Unix(0, int64(f*1e9+0.5))
		if earliestVideoTime.IsZero() || tm.Before(earliestVideoTime) {
			earliestVideoTime = tm
		}
		if tm.After(latestVideoTime) {
			latestVideoTime = tm
		}
		return true
	})
